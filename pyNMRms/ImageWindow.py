...
'''
VersionID='ImageWIndow V12-1-2023'
...
class ImageWindow(QMainWindow):
      def __init__(self, daughter=None, parent = None):
        ...
        self.programID='TNMR RawDataViewer: '
        ...
        self.FoVY=160
        self.xscale=1       #=1 if image displays voxels, =FoV/#voxels if distance display
        self.yscale=1 
...
#         self.actionExportImage.triggered.connect(self.exportImages)
        ...
        self.infv2 = pg.InfiniteLine(movable=True, angle=90, pen=self.p2, label='x={value:0.2f}', 
                       labelOpts={'position':10, 'color': (200,100,100), 'fill': (200,200,200,50), 'movable': True})
        self.infh1 = pg.InfiniteLine(movable=True, angle=0, pen=self.p3,  hoverPen=(0,200,0), label='y={value:0.2f}', 
                   labelOpts={'color': (200,0,0), 'movable': True, 'fill': (0, 0, 200, 100)})
        self.infh2 = pg.InfiniteLine(movable=True, angle=0, pen=self.p4,  hoverPen=(0,200,0), label='y={value:0.2f}', 
                   labelOpts={'color': (200,0,0), 'movable': True, 'fill': (0, 0, 200, 100)})
      ...
      def openTNTFile(self):
        ...
        self.tntComment=self.tntfile.TNMRComment
      ...
      def showTNTComment(self): 
          ...
          self.commentTextBox.info.setText(self.tntComment)
...
#        self.pw.message('Scaled image stack using scipy.ndimage.zoom, scalex={:.2f}, scaley={:.2f}, scalez={:.2f} ,interpolation=cubic spline image shape= {},{},{} \n'.format(scalex,scaley,scalez,s[0],s[1],s[2]))
      ...
      def writeDicomFiles(self, filename):
            nimages=self.rawData.shape[0]
            ncolumns=self.rawData.shape[1]
            nrows=self.rawData.shape[2]
            ...
            if self.dataType=='RawMag':
                    dData=np.absolute(self.rawData)
            if self.dataType=='RawPhase':
                    dData=np.angle(self.rawData)   
            if f[0]=='':
                return 'cancel'
...
#                 pixel_array=np.transpose(self.PA[i])        #numpy    pixel array
      ...
      def makeImageStack(self): 
            """Makes a image stack from current plot, replicates a stack of DICOM images and headers"""
            nslice=self.sliceIndex
            self.iS=ImageList.ImageList()
            for i in range (self.rawData.shape[3]):
                ...
                self.iS.bValue.append(self.bValue)
            ...
            try:
                if self.imv.view.sceneBoundingRect().contains(pos):
                    ...
                    if self.Xindex>=0 and self.Xindex<im.shape[0] and self.Yindex>=0 and self.Yindex<im.shape[1]:
                        self.statusBar.showMessage('Slider_{}{}={:.2f}{}; X={}={}; Y={}={}; Value={:.2f}, Parameter={}, dH(mm)={:.2f}, dV(mm)={:.2f}' \
                            .format(sliderDim, sliderindex, sliderArrayValue,sliderArrayDim, xDim, self.Xindex, yDim, self.Yindex, im[self.Xindex,self.Yindex], self.paramIndex, self.markerdH, self.markerdV),5000)#
            except:
                pass
                #raise 
...
if __name__ == '__main__':
    ...
    imageWindowd = ImageWindow()
    imageWindow = ImageWindow(daughter=imageWindowd)
    imageWindow.show()
    sys.exit(app.exec_())
